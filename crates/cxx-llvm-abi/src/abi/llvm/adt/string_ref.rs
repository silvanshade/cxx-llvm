...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["llvm", "adt", "string_ref"];
    let path_descendants = vec![];
    let cxx_include = "cxx-llvm-abi/cxx/include/llvm/ADT/StringRef.hxx";
    let cxx_namespace = "cxx_llvm::llvm::adt::string_ref";
    let cxx_name = "StringRef";
    let rust_name = "StringRef";
    let lifetimes = ::cxx_memory_abi::indexmap::IndexMap::from_iter([("a", vec![])].into_iter());

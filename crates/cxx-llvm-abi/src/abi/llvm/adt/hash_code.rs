...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["llvm", "adt", "hash_code"];
    let path_descendants = vec![];
    let cxx_include = "cxx-llvm-abi/cxx/include/llvm/ADT/HashCode.hxx";
    let cxx_namespace = "cxx_llvm::llvm::adt::hash_code";
    let cxx_name = "HashCode";
    let rust_name = "HashCode";

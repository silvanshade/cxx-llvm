...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["llvm", "adt", "triple"];
    let path_descendants = vec![];
    let cxx_include = "cxx-llvm-abi/cxx/include/llvm/ADT/Triple.hxx";
    let cxx_namespace = "cxx_llvm::llvm::adt::triple";
    let cxx_name = "Triple";
    let rust_name = "Triple";
